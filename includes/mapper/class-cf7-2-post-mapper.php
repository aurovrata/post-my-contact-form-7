...
abstract class Form_2_Post_Mapper {
	...
	public function save_form_2_post($submission){
		...
		if(isset($_POST['_map_author']) && is_numeric($_POST['_map_author'])){
			...
			//try to get a user form the form mail recipient if no one logged in.
			//get_post_meta ( int $post_id, string $key = '', bool $single = false )
		...
		$post_status = apply_filters('cf7_2_default_post_status_'.$this->post_properties['type'], 'draft', $this->cf7_key, $cf7_form_data);

		if($is_submitted){ //allow programs to publish directly.
			/**
			 * Filter the post status of the custom post created when a form is submitted, default ot 'draft';
			 * @since 2.0.2
